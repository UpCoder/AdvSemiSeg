...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, slice_num, gpu_id=None, print_info=False):
        self.inplanes = 64
        zoom_ratio = 4
        ...
        from CLSTM.BiCLSTM import BiCLSTM
        self.bi_clstm_layer1 = BiCLSTM(input_channels=int(self.growth_rate * 4 / extract_ratio),
                                       hidden_channels=[int(self.growth_rate * 2 / extract_ratio)], kernel_size=3,
                                       step=slice_num, effective_step=range(slice_num))
        self.bi_clstm_layer2 = BiCLSTM(input_channels=int(self.growth_rate * 8 / extract_ratio),
                                       hidden_channels=[int(self.growth_rate * 4 / extract_ratio)], kernel_size=3,
                                       step=slice_num, effective_step=range(slice_num))
        self.bi_clstm_layer3 = BiCLSTM(input_channels=int(self.growth_rate * 16 / extract_ratio),
                                       hidden_channels=[int(self.growth_rate * 8 / extract_ratio)], kernel_size=3,
                                       step=slice_num, effective_step=range(slice_num))
        self.bi_clstm_layer4 = BiCLSTM(input_channels=int(self.growth_rate * 32 / extract_ratio),
                                       hidden_channels=[int(self.growth_rate * 16 / extract_ratio)], kernel_size=3,
                                       step=slice_num, effective_step=range(slice_num))

        self.layer4_upsample = UpSample_Module(int(self.growth_rate * 16 / extract_ratio),
                                               int(self.growth_rate * 16 / extract_ratio), 51, stride=1)

        self.layer3_upsample_conv = nn.Conv2d(int(self.growth_rate * 24 / extract_ratio),
        ...
        self.layer2_upsample_conv = nn.Conv2d(int(self.growth_rate * 12 / extract_ratio),
        ...
        self.layer1_upsample_conv = nn.Conv2d(int(self.growth_rate * 6 / extract_ratio),
    ...
    def forward(self, x):
        ...
        down_layer4_outputs = torch.cat(layer4s, dim=0).unsqueeze(0)
        down_layer3_outputs = torch.cat(layer3s, dim=0).unsqueeze(0)
        down_layer2_outputs = torch.cat(layer2s, dim=0).unsqueeze(0)
        down_layer1_outputs = torch.cat(layer1s, dim=0).unsqueeze(0)
        if self.print_info:
            print('down layer4 outputs size: ', down_layer4_outputs.size())
            print('down layer3 outputs size: ', down_layer3_outputs.size())
            print('down layer2 outputs size: ', down_layer2_outputs.size())
            print('down layer1 outputs size: ', down_layer1_outputs.size())

        up_layer4_input = self.bi_clstm_layer4(down_layer4_outputs)
        if self.print_info:
            print('the up layer4 input size is ', up_layer4_input.size())
        up_layer4_output = self.layer4_upsample(up_layer4_input)

        up_layer3_input = self.bi_clstm_layer3(down_layer3_outputs)
        if self.print_info:
            print('layer3, before cat, ', up_layer3_input.size(), up_layer4_output.size())
        up_layer3_input = torch.cat([up_layer3_input, up_layer4_output], dim=1)
        if self.print_info:
            print('the up_layer3 input size is ', up_layer3_input.size())
        up_layer3_input = self.layer3_upsample_conv(up_layer3_input)
        up_layer3_output = self.layer3_upsample(up_layer3_input)

        up_layer2_input = self.bi_clstm_layer2(down_layer2_outputs)
        if self.print_info:
            print('layer2, before cat, ', up_layer2_input.size(), up_layer3_output.size())
        up_layer2_input = torch.cat([up_layer2_input, up_layer3_output], dim=1)
        if self.print_info:
            print('the up_layer2 input size is ', up_layer2_input.size())
        up_layer2_input = self.layer2_upsample_conv(up_layer2_input)
        up_layer2_output = self.layer2_upsample(up_layer2_input)

        up_layer1_input = self.bi_clstm_layer1(down_layer1_outputs)
        if self.print_info:
            print('layer1, before cat, ', up_layer1_input.size(), up_layer2_output.size())
        up_layer1_input = torch.cat([up_layer1_input, up_layer2_output], dim=1)
        if self.print_info:
            print('the up_layer1 input size is ', up_layer1_input.size())
        up_layer1_input = self.layer1_upsample_conv(up_layer1_input)
        up_layer1_output = self.layer1_upsample(up_layer1_input)

        layer_5 = self.layer5(up_layer1_output)
        #
